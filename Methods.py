...
def universal_split(text_string, regex_to_find, regex_to_replace_for_the_name, name):
    result_dict = {}
    current_key = None

    pattern = re.compile(regex_to_find)
    pattern_replacing = re.compile(regex_to_replace_for_the_name)

    for line in text_string.splitlines():
        match = pattern.search(line)
        if match:
            current_key = re.sub(regex_to_replace_for_the_name, "", match.group(0).strip())
            current_key = f"{name}{current_key}"
            while current_key in result_dict:
                current_key = f"{current_key}*"
            result_dict[current_key] = "\n" + line[len(match.group(0)):].strip()
        elif line.strip() != "":
